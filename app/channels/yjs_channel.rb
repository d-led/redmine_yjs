...
#
# Only functional when ActionCable is available (not required for direct WebSocket mode)

# Check for ActionCable availability directly (ApplicationCable::Channel may not be loaded yet)
if defined?(ActionCable::Channel::Base)
  # Ensure ApplicationCable::Channel exists (it's defined in connection.rb, but may not be loaded yet)
  # This handles load order issues with Zeitwerk autoloading - Ruby's module keyword will
  # create the module if it doesn't exist, or reopen it if it does
  unless defined?(ApplicationCable::Channel)
  ...
  # Stub class when ActionCable isn't available to satisfy Zeitwerk autoloader
  class YjsChannel
    def self.method_missing(method_name, *args, &block)
      Rails.logger.debug "[Yjs] YjsChannel.#{method_name} called but ActionCable not available"
      nil
    end
